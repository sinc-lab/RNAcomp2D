...
def run_method(sequence, params, temp_dir, ref=None):
    """Calls IPknot method from IPknot folder.
    ...
    inputfile = ut.generateFasta("IPknot", sequence, f"{temp_dir}")
    command = ["docker", "run", "--rm", "-v", f"{full_temp_dir}:/app", 
               "ipknot", "results/IPknot.fasta"]
    val = subprocess.run(command, stdout=subprocess.PIPE, 
                         stderr=subprocess.PIPE, universal_newlines=True)
    if val.returncode == 0:
        lines = val.stdout.splitlines()
        with open(f"{temp_dir}/results/IPknot.dot", 'w') as f:
            f.write("IPknot\n")
            f.write(sequence + "\n")
            f.write(lines[-1] + "\n")
        ...
        if ref is not None:
            ...
            with open(f"{temp_dir}/results/IPknot_conn.txt", 'w') as f:
        ...
        draw_val1 = ut.draw("IPknot", temp_dir, color_str)
        draw_val2 = ut.draw_circ("IPknot", temp_dir)
        if draw_val1.returncode != 0 or draw_val2.returncode != 0:
            return "Sequence folded incorrectly, drawing failed"
        return "OK"
    else:
        return "Error running IPknot"
